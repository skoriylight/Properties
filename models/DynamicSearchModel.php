    ...
    public function search($params)
    {
        ...
        foreach ($this->propertyGroups as $groupId => $properties) {
            foreach ($properties as $key => $propertyValue) {
                /** @var \common\components\properties\PropertyValue $propertyValue */
                ...
                if ($prop->is_column_type_stored) {
                    if ($table_inheritance_joined === false) {
                        ...
                        $query->join('INNER JOIN', $object->column_properties_table_name . ' ti',
                            'ti.entity_model_id = ' . $baseModelTableName . '.id');
                    }
                    ...
                    $query->join(
                        ...
                        $eavTableName.'.entity_model_id = '.$baseModelTableName.".id AND $eavTableName.key=:$key",
                        [$key=>$prop->key]
                    );
                    if ($prop->value_type === 'STRING' && $prop->property_handler_id !== 3) {
                    ...
                    $query->join(
                        'INNER JOIN',
                        "entity_static_value $osvTableName",
                        "$osvTableName.entity_id={$object->id} AND $osvTableName.entity_model_id=$baseModelTableName.id"
                    );
            ...
            foreach ($properties as $key => $propertyValue){
                ...
                if ($prop->has_static_values) {
                    $column = [
                        ...
                        'filter' => PropertyStaticValue::getSelectForPropertyId($propertyValue->property_id),
                    ];
