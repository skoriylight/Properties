...
namespace common\components\properties\url;
...
class PropertyPart extends UrlPart
{
    ...
    public function getNextPart($full_url, $next_part, &$previous_parts)
    {
        ...
        if ($property->has_static_values && $property->has_slugs_in_values) {
            $cacheTags = [];
            $static_values = PropertyStaticValue::getValuesForPropertyId($this->property_id);
            ...
            foreach ($static_values as $value) {
                if ($slugs[$currentSlug] === $value['slug']) {
                    ...
                    $cacheTags[] = ActiveRecordHelper::getObjectTag(PropertyStaticValue::className(), $value['id']);
                    $currentSlug++;
            ...
            if (isset($parameters['properties'][$this->property_id]) &&
                ...
                $psvs = Yii::$app->db->cache(
                    function($db) use ($property_id, $parameters) {
                        ...
                        return PropertyStaticValue::find()
                            ->where(['id' => $vals])
                            ->orderBy('position ASC, name ASC')
                            ->asArray(true)
                            ->all();
                    },
                    86400, new TagDependency(['tags'=>[ActiveRecordHelper::getCommonTag(PropertyStaticValue::className())]])
                );
                ...
                if (!empty($psvs)) {
                    foreach ($psvs as $psv) {
                        $cacheTags[] = ActiveRecordHelper::getObjectTag(PropertyStaticValue::className(), $psv['id']);
                    }
